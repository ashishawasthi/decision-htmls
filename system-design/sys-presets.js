...
(function (NS) {
  ...
  const DECISIONS = [
    ...
    { key: 'reactMaxIter', group: 'Agent', label: 'ReAct max iterations', type: 'steps', min: 1, max: 12, step: 1, vis: onMulti, help: 'Maximum reason-act-observe loops allowed per request; drives the model-call fan-out and caps the revise loop.' },
    ...
    { key: 'smartRouting', group: 'Models & routing', label: 'Smart routing', type: 'bool', vis: d => !noAgent(d), help: 'Send simple requests to the fast model, hard ones to the reasoning model.' },
    ...
    /* Retrieval */
    { key: 'retrieval', group: 'Retrieval', label: 'Strategy', type: 'enum', options: [{ v: 'none', label: 'None' }, { v: 'dense', label: 'Pure dense' }, { v: 'hybrid', label: 'Hybrid (dense+BM25)' }, { v: 'rerank', label: 'Hybrid + reranker' }], vis: d => !noAgent(d), help: 'Grounding retrieval strategy at query time. Auto turns it on exactly when an indexed source (corpus, website) is selected, and drops the reranker at a sub-second SLO.' },
