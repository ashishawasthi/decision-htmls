...
(function (NS) {
  ...
  const DECISIONS = [
    ...
    { key: 'stateStore', group: 'State & context cache', label: 'State store', type: 'enum', options: [{ v: 'alloydb', label: 'AlloyDB' }, { v: 'redis_alloydb', label: 'Redis + AlloyDB' }, { v: 'spanner', label: 'Spanner' }, { v: 'redis_spanner', label: 'Redis + Spanner' }, { v: 'cloudsql', label: 'Cloud SQL' }, { v: 'redis', label: 'Memorystore (Redis Cluster)' }], vis: d => !noAgent(d), help: 'Where transactional run and agent state is kept. Auto: a Redis hot tier + AlloyDB for assistants, plain AlloyDB for automation; the durable tier becomes Spanner when multi-region active-active is on. Audit history goes to BigQuery either way. The no-agent path has no run state (Agent Search manages sessions).' },
