...
(function (NS) {
  ...
  const PRESETS = {
    assistant: {
      expert_copilot: {
        ...
        desc: 'Internal expert copilot that answers from a document corpus and the application\'s own AlloyDB database (current + historical data) at agentic speed.',
        inputs: inp({ dataSources: ['alloydb_oltp', 'doc_corpus'], freshness: 'minutes' }),
      },
      self_managed: {
        label: 'Self-Managed',
      ...
      enterprise_search: {
        ...
        inputs: inp({ actors: 20000, actionsPerDay: 12, burst: 6, activeHoursPerWeek: 60, latencyPreset: 'subsecond', tokensIn: 2000, tokensOut: 150, dataSources: ['doc_corpus', 'website'], corpusSize: 1e7, freshness: 'eod' }),
      },
  ...
  const INPUT_HELP = {
    ...
    latencyPreset: 'Target response-time SLO; each tier derives a different architecture. Sub-second: no agent at all - Agent Search answers directly from its index (bundled grounded answer, no external model call, no tools). Interactive (<2s start / <5s full): one streaming single agent with grounding and tools. Agentic (<10s start / <12s full): the validator-gated multi-agent team. Incompatible sources left selected are flagged instead of removed.',
    ...
    languages: 'Languages the system must understand and respond in. CJK (ZH, JA) runs ~2-3 tokens per word vs ~1.3 for English, so corpus ingestion math (index size, embedding and de-identification volume) scales ~1.8x; chunk on character boundaries and evaluate on native-language golden sets, not translated English.',
  };
  ...
  const noAgent = d => d.pattern === 'none';
  const onSelfbuilt = d => d.retrieval !== 'none' && d.ragEngine === 'selfbuilt';
  const onSelfhost = d => (d.reasoningModel === 'llama4-selfhost' || d.fastModel === 'llama4-selfhost') && !noAgent(d);
  ...
  const DECISIONS = [
    ...
    /* Agent */
    { key: 'agentRuntime', group: 'Agent', label: 'Agent compute', type: 'enum', options: [{ v: 'agentengine', label: 'Agent Runtime (managed)' }, { v: 'gke', label: 'GKE Autopilot' }], vis: d => !noAgent(d), help: 'Where the agent runs. Auto follows the operating model and model strategy: managed Agent Runtime unless the platform is self-managed or a self-host model needs an in-VPC fleet.' },
    { key: 'pattern', group: 'Agent', label: 'Pattern', type: 'enum', options: [{ v: 'none', label: 'No agent (Agent Search QA)' }, { v: 'single', label: 'Single agent' }, { v: 'multi', label: 'Multi-agent' }], help: 'No agent (Agent Search answers directly from its index - no external model call, no tools), one streaming agent, or a coordinated validated team. Auto follows the SLO: sub-second = no agent, interactive = single, agentic = multi.' },
    ...
    { key: 'reactMaxIter', group: 'Agent', label: 'ReAct max iterations', type: 'steps', min: 1, max: 12, step: 1, vis: onMulti, help: 'The loop GUARD: maximum reason-act-observe steps allowed per request, set ~1.5x above the expected 5-8 steps. Pair it with a max-spend-per-case cap; hitting either guard escalates the case with its trace attached. Drives the model-call fan-out and caps the revise loop.' },
    { key: 'reviseRate', group: 'Agent', label: 'Draft revise rate %', type: 'steps', min: 0, max: 60, step: 5, vis: onMulti, help: 'Share of drafts the Validator rejects, triggering a regenerate-and-revalidate cycle. The p95 latency pays every extra cycle still likelier than 5% (floor(log 0.05 / log rate)), capped at one below the iteration cap.' },
    { key: 'platform', group: 'Agent', label: 'Platform', type: 'enum', options: [{ v: 'adk', label: 'ADK' }, { v: 'studio', label: 'Agent Studio' }, { v: 'langgraph', label: 'LangGraph' }], vis: d => !noAgent(d), help: 'Agent framework and runtime that hosts the system.' },
    /* Models and routing */
    { key: 'reasoningModel', group: 'Models & routing', label: 'Model for high reasoning', type: 'enum', options: 'MODELS', vis: d => !noAgent(d), help: 'Primary model for the hard reasoning steps. Auto follows the model strategy.' },
    { key: 'fastModel', group: 'Models & routing', label: 'Model for fast lookups', type: 'enum', options: 'MODELS', vis: d => !noAgent(d), help: 'Cheaper, faster model for simple or lookup steps. Auto follows the model strategy.' },
    { key: 'smartRouting', group: 'Models & routing', label: 'Smart routing', type: 'bool', vis: d => !noAgent(d), help: 'Send simple requests to the fast model, hard ones to the reasoning model - policy in configuration, not code. Never downgrade silently: every degraded route is explicit, logged, and visible in response metadata, and safety stages are never skipped to save latency.' },
    { key: 'routingSplit', group: 'Models & routing', label: '% to fast model', type: 'steps', min: 0, max: 100, step: 5, vis: d => d.smartRouting && !noAgent(d), help: 'Share of traffic routed to the fast model. Auto biases to 85% at a sub-second SLO, 70% otherwise.' },
    { key: 'judgeDiversity', group: 'Models & routing', label: 'Cross-model judge diversity', type: 'bool', vis: d => !noAgent(d), help: 'Use a different model to check or judge outputs.' },
    /* Retrieval */
    { key: 'retrieval', group: 'Retrieval', label: 'Strategy', type: 'enum', options: [{ v: 'none', label: 'None' }, { v: 'dense', label: 'Pure dense' }, { v: 'hybrid', label: 'Hybrid (dense+BM25)' }, { v: 'rerank', label: 'Hybrid + reranker' }], vis: d => !noAgent(d), help: 'Grounding retrieval strategy at query time. Hybrid fuses dense + keyword with reciprocal rank fusion (the keyword leg catches exact IDs and codes embeddings miss); the cross-encoder reranker is a named trade - ~80ms at p95 for roughly half the hallucination rate. Auto turns retrieval on exactly when an indexed source is selected, and drops the reranker at a sub-second SLO.' },
    { key: 'ragEngine', group: 'Retrieval', label: 'Indexing & retrieval engine', type: 'enum', options: [{ v: 'vais', label: 'Managed: Agent Search' }, { v: 'selfbuilt', label: 'Self-built pipeline + managed store' }], vis: d => d.retrieval !== 'none' && !noAgent(d), help: 'Managed Agent Search (parse, chunk, embed, index, retrieve, rerank in one service), or a self-built ingestion and query pipeline on top of a managed ScaNN-backed vector store.' },
    ...
    { key: 'metadataPrefilter', group: 'Retrieval', label: 'Metadata pre-filter', type: 'bool', vis: onSelfbuilt, help: 'Filter candidates by metadata before the vector search - business line, region, language, and the access-control principals. Entitlement filters BEFORE ranking, so the model never sees a forbidden chunk: access control lives in the index, not in a prompt.' },
    { key: 'ingestionSep', group: 'Retrieval', label: 'Ingestion separation', type: 'bool', vis: onSelfbuilt, help: 'Index on a separate pipeline from query serving, so the two planes scale independently - and so a re-index storm (an embedding model swap re-embeds the whole corpus; vector spaces do not mix) never touches query latency. Auto turns this on for a 5M+ document corpus.' },
    { key: 'dlpDeidIngest', group: 'Retrieval', label: 'DLP de-identify at ingest', type: 'bool', vis: (d, i) => d.retrieval !== 'none' && ((i && i.dataSources) || []).includes('doc_corpus'), help: 'Run Sensitive Data Protection de-identification on parsed documents BEFORE anything is embedded or indexed - PII that reaches the vectors and the index replays in every retrieved chunk. Auto turns it on for regulated and strict-PII tiers. Agent Search has no built-in de-identification, so the managed path pre-processes the corpus before import; crawled public website pages skip it.' },
    /* State and context cache */
    { key: 'stateStore', group: 'State & context cache', label: 'State store', type: 'enum', options: [{ v: 'managed', label: 'Managed (Agent Runtime)' }, { v: 'redis', label: 'Redis on GKE' }, { v: 'redis_spanner', label: 'Redis + Spanner' }, { v: 'alloydb', label: 'AlloyDB' }, { v: 'redis_alloydb', label: 'Redis + AlloyDB' }, { v: 'spanner', label: 'Spanner' }, { v: 'cloudsql', label: 'Cloud SQL' }], vis: d => !noAgent(d), help: 'Where run and session state is kept. Auto: managed Agent Runtime persists it in Sessions + Memory Bank (no database to provision); self-managed on GKE brings its own Redis (which persists via AOF/RDB and recovers from a replica), with Spanner added for active-active multi-region. AlloyDB / Cloud SQL stay pinnable if you want a relational tier. Audit history goes to BigQuery either way. The no-agent path has no run state (Agent Search manages sessions).' },
    { key: 'contextCache', group: 'State & context cache', label: 'Context cache (system prompt)', type: 'bool', vis: d => !noAgent(d), help: 'Bill the reusable prompt prefix at the cache-read rate (~10% of input). Gemini caches a stable prefix implicitly; Claude needs a cache_control breakpoint; self-hosted Llama uses vLLM prefix caching. Needs a prefix above the ~2k-token minimum cacheable size, which is what Auto checks.' },
    { key: 'reuseInputPct', group: 'State & context cache', label: 'Reusable / cacheable input %', type: 'steps', min: 0, max: 90, step: 5, vis: d => d.contextCache && !noAgent(d), help: 'Share of input that is a stable, cacheable prefix. Realized by ordering the prompt fixed-part-first: system instructions, context, and few-shot at the start, the variable user turn at the end.' },
    /* Response caching */
    ...
    { key: 'semanticCache', group: 'Response caching', label: 'Semantic cache', type: 'bool', help: 'Reuse a stored answer when a similar query repeats. Namespaced per tenant, never global: a shared semantic cache can serve one tenant an answer generated from another tenant\'s context.' },
